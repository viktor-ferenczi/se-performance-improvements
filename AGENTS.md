...
Also, read the project's `README.md` to understand its purpose and context, and the
[Developer Handbook](Docs/TOC.md) (`Docs/TOC.md`) for a module-by-module, file-by-file
map of the codebase. The handbook is generated by the `structured-documentation` skill;
its generator scripts and working data live under `Docs/data/` and can be re-run to
