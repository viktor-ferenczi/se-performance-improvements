...
{
    // Cache for the lookup tables generated for each method
    private static Dictionary<int, Dictionary<Type, int>> _methodLuts = new Dictionary<int, Dictionary<Type, int>>();
    private static int _nextLutId = 0;

    public static void ApplyPatch(Harmony harmony, MethodInfo originalMethod)
    {
        var processor = harmony.CreateProcessor(originalMethod);
        processor.AddTranspiler(AccessTools.Method(typeof(RuntimeOptimizer), nameof(Transpiler)));
        processor.Patch();
    }
    ...
    {
        if (_methodLuts.TryGetValue(lutId, out var lut))
        {
            if (lut.TryGetValue(type, out int index))
            {
                return index;
            }
        }
    ...
    public static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions, MethodBase originalMethod)
    {
        ...
        if (typeChecks.Count < 5) // Threshold to apply optimization
            return codes;
        ...
        for (int i = 0; i < typeChecks.Count; i++)
        {
            ...
            {
                var lastCheck = currentChain.Last();
                // Check if lastCheck.FailLabel points to typeChecks[i].StartIndex
                if (LabelsMatch(codes, lastCheck.FailLabel, typeChecks[i].StartIndex))
                {
        ...
        // Call GetTypeIndex
        newInstructions.Add(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(RuntimeOptimizer), nameof(GetTypeIndex))));
        // Switch
