...
A Prefix on the getter returns the cached value while it is younger than a second of simulation ticks; otherwise the original runs and the Postfix stores its result. The `EnsureCode` hash lists both the shipped body and the body linux-compat's preloader substitutes on Linux. Gated by `FixMemoryStats`.
