...
Two configuration options drive it: `HavokThreadCountMode` (`Game`, `Auto` or `Manual`) and `HavokThreadCount`. In `Game` — the server default — the patch answers nothing and the game's own sizing stands, exactly as without the plugin. In `Auto` the count is `HavokThreads.Auto` — one worker per physical CPU core minus one, on both platforms, so hyper-threading siblings are not counted and the main thread keeps a core of its own — and `Configure()` writes it back into `Config.HavokThreadCount`, so the in-game dialog and the server's web UI always show the count the game is going to get. On the client the config enforces this itself: selecting `Auto` sets the count to `HavokThreads.Auto`, the count cannot be set to anything else while `Auto` is on, and the dialog disables the slider in every mode but `Manual`. In `Manual` the configured number is used, clamped to `HavokThreads.Min`..`HavokThreads.Max` (2..64). With the plugin disabled the patch answers nothing either.
