...
The patches are applied in phases on both sides, so verification is scoped to match. `VerifyUncategorized()` checks only the patch classes *without* a Harmony category (mirroring `PatchAllUncategorized`); `VerifyCategory(category)` checks only those in the given category (the `"Early"` phase applied from the preloader bootstrap, and the deferred `"Late"` phase). Each phase resolves only the game methods it is about to patch, so a patch whose target assembly is not loaded yet — handled in a later phase — does not cause a spurious failure here. These overloads resolve the patch assembly directly (`typeof(EnsureCode).Assembly`, which is the plugin assembly because the `Shared` project is compiled into it), whereas the parameterless `Verify()` finds the calling assembly via the call stack.
...
| `Verify()` *(static)* | Method | Entry point for single-phase startup; scans the calling assembly and returns all detected [`CodeChange.cs`](CodeChange.cs.md) mismatches. Unused by either plugin today, both apply the patches in phases. |
| `VerifyUncategorized()` *(static)* | Method | Verifies only the uncategorized patch classes; mirrors `Harmony.PatchAllUncategorized`. |
