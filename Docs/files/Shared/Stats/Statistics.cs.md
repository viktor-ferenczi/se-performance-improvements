...
| `Capture()` | Static method | Creates a fresh `StatisticsSnapshot` and fills it via the `CaptureStatistics` methods of `MySafeZonePatch`, `MySessionComponentSafeZonesPatch`, `MyWindTurbinePatch`, `MyGridConveyorSystemPatch`, `MyAssemblerPatch` and `MyToolbarItemTerminalGroupPatch`, resetting their counters. |
