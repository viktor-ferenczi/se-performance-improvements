...
| `Capture()` | Static method | Creates a fresh `StatisticsSnapshot` and fills it via the `CaptureStatistics` methods of `MySafeZonePatch`, `MySessionComponentSafeZonesPatch`, `MyWindTurbinePatch`, `MyGridConveyorSystemPatch`, `MyAssemblerPatch`, `MyToolbarItemTerminalGroupPatch` and `MyGridTerminalSystemPatch`, resetting their counters. |
