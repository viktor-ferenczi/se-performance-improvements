...
**Derived cache.** [`MyAssemblerPatch.cs`](../files/Shared/Patches/Conveyor/MyAssemblerPatch.cs.md) does not use `ReachableCaches`; it keeps its own per-assembler list of reachable assemblers and stamps each entry with `MyGridConveyorSystemPatch.Generation`, a counter the cache core bumps from `InvalidateCache`, `DropCache` and a Postfix on `MyGridConveyorSystem.FlagForRecomputation`. The same events that clear the pairwise cache therefore discard the assembler entries, and the recomputation hook adds the conveyor line working state changes the pairwise cache never followed. Both share the `FixConveyor` toggle.

**Inactive code.** Three pathfinding files ([`MyPathFindingSystemPatch.cs`](../files/Shared/Patches/Conveyor/MyPathFindingSystemPatch.cs.md), [`MyPathFindingSystemEnumeratorPatch.cs`](../files/Shared/Patches/Conveyor/MyPathFindingSystemEnumeratorPatch.cs.md), [`MyPathfindingDataPatch.cs`](../files/Shared/Patches/Conveyor/MyPathfindingDataPatch.cs.md)) are disabled because patching generic types with Harmony is no longer reliable. [`PullItemStats.cs`](../files/Shared/Patches/Conveyor/PullItemStats.cs.md) is active only in `#if DEBUG` builds. None of these affect release performance.
