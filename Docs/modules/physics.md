...
The first is the size of the Havok job thread pool. `MyPhysics.LoadData` takes it from `MyVRage.Platform.System.OptimalHavokThreadCount`, which the game answers with a hard `null`, so Havok sizes the pool from the machine on its own terms. [`MyWindowsSystemPatch.cs`](../files/Shared/Patches/Physics/MyWindowsSystemPatch.cs.md) answers that property instead: either automatically (`min(16, processorCount)`, the default) or with the number the operator configured. It replaced a transpiler on `MyPhysics.LoadData` which wrote an `int` into a `Nullable<int>` local — invalid IL, disabled on .NET Core because it corrupted memory during world load.
