...
The reusable toolbox the patches build on: time-bounded caches, reader/writer-locked collections, Harmony IL/transpiler helpers, IL verification, hashing, object pools, the mod-rewriter detection behind the compilation cache keys and the assembly publicizer support types.

The `Shared/Tools` module is the foundation layer that every performance patch in the plugin builds on. It is divided into six theme groups:

**Caching** — [`Cache.cs`](../files/Shared/Tools/Cache.cs.md), [`TwoLayerCache.cs`](../files/Shared/Tools/TwoLayerCache.cs.md), and [`UintCache.cs`](../files/Shared/Tools/UintCache.cs.md) are the three generic cache primitives; each is a time-bounded key/value store backed by a [`RwLockDictionary.cs`](../files/Shared/Tools/RwLockDictionary.cs.md). `Cache` is the workhorse for most per-entity caches (safe zones, wind turbines, block-access rights). `TwoLayerCache` adds a lock-free immutable read path for look-up-heavy workloads. `UintCache` eliminates per-entry heap allocation by packing a `uint` value and expiry tick into a single `ulong`. [`CacheStat.cs`](../files/Shared/Tools/CacheStat.cs.md) is the per-cache lookup/hit counter behind the runtime statistics ([`Statistics.cs`](../files/Shared/Stats/Statistics.cs.md) in [shared-plugin-core](shared-plugin-core.md)) and the DEBUG hit-rate reports; [`ConveyorStat.cs`](../files/Shared/Tools/ConveyorStat.cs.md) is its call-count counterpart for the conveyor reachability investigation patches.
...
**Compilation cache keys** — [`ModRewriterVersions.cs`](../files/Shared/Tools/ModRewriterVersions.cs.md) detects the loaded mod-rewriting plugins (those whose `IPlugin` main type declares a `Rewrite` method) and hashes their module version IDs, so [`MyScriptCompilerPatch.cs`](../files/Shared/Patches/ScriptCompiler/MyScriptCompilerPatch.cs.md) can fold the exact rewriter builds into its on-disk compilation cache keys. [`LegacyModRewriters.cs`](../files/Shared/Tools/LegacyModRewriters.cs.md) is its fallback for the compat plugin builds loaded by old Pulsar and Magnetar releases, which predate the `Rewrite` hook; it is scheduled for removal.
...
| [`CacheStat.cs`](../files/Shared/Tools/CacheStat.cs.md) | Per-cache lookup/hit counter, sampled into the runtime statistics and the DEBUG hit-rate reports |
...
| [`LegacyModRewriters.cs`](../files/Shared/Tools/LegacyModRewriters.cs.md) | Legacy fallback recognizing pre-`Rewrite`-hook dotnet-compat / linux-compat builds by their compiler-hook marker types |
| [`ModRewriterVersions.cs`](../files/Shared/Tools/ModRewriterVersions.cs.md) | Detects loaded mod-rewriting plugins and hashes their module version IDs into the compilation cache keys |
...
**Compilation cache keys guard the world-loading cache.** [`ModRewriterVersions.cs`](../files/Shared/Tools/ModRewriterVersions.cs.md)`.Initialize()` must run before any mod or script compilation: on the dedicated server [`Preloader.cs`](../files/ServerPlugin/Preloader.cs.md) calls it from the loader's post-hook and [`Common.cs`](../files/Shared/Plugin/Common.cs.md)`.SetPlugin` re-enters it idempotently and logs the result; on the client only the `SetPlugin` call exists. It scans the loaded assemblies for `IPlugin` types declaring `Rewrite`, falls back to [`LegacyModRewriters.cs`](../files/Shared/Tools/LegacyModRewriters.cs.md) when none do, and exposes a SHA1 over the sorted MVIDs as `Hash`, which `MyScriptCompilerPatch.GetScriptsHash` XORs into the cache key. It throws if no `IPlugin` implementation is loaded at all, since silently skipping would key the caches wrongly.
